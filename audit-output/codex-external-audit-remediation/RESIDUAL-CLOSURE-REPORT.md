...
- Live-evidence decision: **EXECUTED (owner-authorized) AND CLOSED 2026-08-30 — council-advantage claim RETIRED (ACCEPTED-DOC).** Pre-registered repeated run (3 × 20 × 2 = 120 runs, glm-5.3): criteria 1-5 all MET (48/48 blocking, 0 forbidden-invention, 15/15 G1, complete usage, council cost 2.04× ≤ 3×) but the binding sign-test NOT MET (36 greenfield pairs, 9 discordant < 10 required; council won 8 of 9; one-sided exact p=0.0195) → by the pre-registered rule the claim is NOT substantiated. Council is labeled EXPERIMENTAL, single stays default, product positioned as a validated spec compiler. Sanitized result: `audit-output/eval/LIVE-EVAL-RESULT-2026-08-30.md`; total spend ≈ 7.36M tokens (owner subscription); owner-private artifacts (21 generated spec bundles, 120 unit records) at `eval-live-output/` (untracked).
...
| Live eval authorization (cost envelope) | **AUTHORIZED AND EXECUTED** (owner gate 2026-08-28) — decision NOT MET → claim retired | 2026-08-28/30 | LIVE-EVAL-RESULT-2026-08-30.md (aggregate, anonymized) |
...
- **General external pilot: READY WITH DOCUMENTED CONDITIONS** — (1) council-advantage claim RETIRED by the executed pre-registered live run (single is default; council experimental; re-testing requires a NEW pre-registered experiment); (2) untrusted-client exposure requires the exec root pin + review of the documented TOCTOU residual; (3) live generation measured at ~5-40 min/spec with reasoning models (usage guidance needed for UX).
- **Production: READY** — U1 attestation recorded (FIXED), bundle deleted, claim retired with evidence; ship as a single-model spec compiler with council as an explicitly experimental option.
