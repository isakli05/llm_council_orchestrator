...
describe('L01_UNDEFINED_TERM', () => {
  it('fires exactly L01 on the L01 vector (unknown glossary term in the message)', () => {
    ...
    const ruleErrors = result.errors.filter((f) => f.rule === 'L01_UNDEFINED_TERM');

    expect(ruleErrors.length).toBeGreaterThan(0);
    expect(new Set(ruleErrors.map((f) => f.rule))).toEqual(
      new Set(['L01_UNDEFINED_TERM']),
    );
    expect(ruleErrors.some((f) => f.message.includes('Queue System'))).toBe(true);
  });
  ...
  it('reports the requirement id as the finding path', () => {
    ...
    expect(ruleErrors.every((f) => f.path === 'REQ-0002')).toBe(true);
  });
});
