...
describe('regeneration — append-only history', () => {
  ...
  it('the committed lock history is append-only: the original pre-registration entry survives verbatim', () => {
    ...
    // and after the 2026-08-28 corpus substitution
    expect(lock.history.length).toBeGreaterThanOrEqual(3);
    ...
    // entry 2 (2026-08-27) records the rubric-scope extension and the hash it
    // superseded
    expect(lock.history[1]!.note).toMatch(/prompts\+checker|rubric/i);
    expect(lock.history[1]!.previous_hash).toBe(lock.history[0]!.hash);
    // the enforced (newest) entry is the 2026-08-28 owner-directed corpus
    // substitution, registered before any live run, chaining onto entry 2
    const newest = lock.history[lock.history.length - 1]!;
    expect(newest.frozen_at).toBe('2026-08-28');
    expect(newest.note).toMatch(/owner-directed|real-workload/i);
  ...
  it('the committed lock is never regenerated by these tests (no accidental history growth)', () => {
    const lock = loadCorpusLock(LOCK_PATH);
    // pinned by the append-only test above: exactly the original freeze, the
    // I-1 scope extension (both 2026-08-27), and the 2026-08-28 corpus
    // substitution — nothing else, and no test above appended to it
    expect(lock.history).toHaveLength(3);
